<template>
  <view class="list-part">
		...
		<view v-if="empty">
			<view class="workList">
				<view v-for="(item,index) in dataList" :key="index" class="box" @click="goto('/securityManage/addRectification/addRectification',{ editId: item.id })">
					<view class="top">
...
<script>
let that = null
import { safeRectifyFindList } from '@/api/lpgSecurityManageApi.js'
...
export default {
  ...
  methods: {
    ...
    async findDataList() {
      ...
      const { returnValue: res, totals } = await safeRectifyFindList(data)
      if (res) {
