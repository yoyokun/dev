<template>
	<view class="search">
		<view class="search-content">
			<view class="box">
				<u-search :height="35" :bgColor="bgColor" :placeholder="$t('search.searchPlaceholder')"
					:showAction="false" v-model="params.keyword" @search="emitSearch" @clear="emitSearch"></u-search>
			</view>
		...
		<u-popup mode="bottom" round="14" customStyle="overflow:hidden" closeable :show="show" @close="close">
			<view class="filter">
					...
					<block v-for="(item,index) in searchOptions" :key="index">
						...
						<view class="cell" v-if="item.type=='text'">
							...
							<view class="cell-box" :style="item.labelText?'margin-top:20rpx':''">
								<u-input :value="params[item.fieldName]" :placeholder="item.placeholder"
									border="surround" clearable @change="changeItem($event,item.fieldName)"></u-input>
							</view>
						</view>
						<!-- 一般选择 单选 -->
						...
						<view class="cell" v-if="item.type=='date'||item.type=='datetime'">
							...
							<view class="cell-box">
								<view class="datetime">
									<uni-datetime-picker :placeholder="item.placeholder" :type="item.type"
										:value="params[item.fieldName]" @change="changeItem($event,item.fieldName)">
									</uni-datetime-picker>
								</view>
							</view>
						</view>
						...
						<view class="cell" v-if="item.type=='daterange'||item.type=='datetimerange'">
							...
							<view class="cell-box">
								<view class="datetime">
									<uni-datetime-picker :value="params[item.fieldName]"
										:start-placeholder="item.placeholder[0]" :end-placeholder="item.placeholder[1]"
										:type="item.type" @change="changeItem($event,item)">
									</uni-datetime-picker>
								</view>
							</view>
						</view>
				...
				<view class="actions">
					<u-button class="button" shape="circle" type="primary" plain @click="reset">
						{{$t('search.resetBtn')}}
					</u-button>
					<u-button class="button" shape="circle" type="primary" @click="confirm">{{$t('search.confirmBtn')}}
					</u-button>
				</view>
			</view>
		</u-popup>
...
<script>
	export default {
		...
		methods: {
			...
			// 选择选项
			selectItem(val, field, multiple = false) {
				if (multiple) {
					if (this.params[field] && this.params[field].length && this.params[field].indexOf(val) > -1) {
						const index = this.params[field].indexOf(val)
						this.params[field].splice(index, 1)
						this.emitSearch()
					} else {
						this.changeItem(val, field, multiple)
					}
				} else {
			...
			// 选择选项
			changeItem(val, field, multiple = false) {
				if (typeof field === 'object') {
					...
					if (multiple) {
						if (this.params[field] && this.params[field].length) {
							this.params[field].push(val)
						} else {
