...
<script>
let that = null
import { riskPollingMyList } from '@/api/lpgSecurityManageApi.js'
  ...
  onLoad() {
	...
	onShow() {
		uni.setNavigationBarTitle({
			title: this.$t('security.userPatrolCheck.titleText')
		})
	},
  methods: {
		async init() {
    ...
    async findDataList() {
      ...
      const { returnValue: res, totals } = await riskPollingMyList(data)
      if (res) {
