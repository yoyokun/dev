<template>
	<view class="container">
		...
		<view class="part">
			...
			<!-- #ifdef APP-PLUS -->
			<view class="form-box" @click="goto('/personalCenter/version/version')">
				<view class="title">
		...
		<!--切换身份-->
		<u-picker :show="show" ref="uPicker" :closeOnClickOverlay="true" :defaultIndex="defaultIndex"
			:columns="inIdentityList" keyName="orgName" @confirm="switchOrg" @close="show = false"
		...
		methods: {
			...
			async switchOrg({
				...
				if (this.userInfo.orgId !== value[0].orgId) {
					const {
						returnValue: res
					} = await sysManagerChangerIdentity({
						identityId: value[0].id
					})
					if (res) {
						location.reload()
					}
